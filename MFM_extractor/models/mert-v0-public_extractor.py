...
class MertV0PublicExtractor:
    ...
    def extract_features(self, audio_path):
        ...
        (This method is for individual processing.)
        """
        try:
        ...
        all_layer_hidden_states = torch.stack(outputs.hidden_states).squeeze(1)  # now (13, time, hidden_dim)
        
        # Average over time dimension -> (13, hidden_dim)
        time_reduced_hidden_states = all_layer_hidden_states.mean(dim=-2)
        
        # Prepare for aggregator: add batch dimension -> (1, 13, hidden_dim)
        time_reduced_hidden_states = time_reduced_hidden_states.unsqueeze(0)
        
        # Aggregator output -> (1, 1, hidden_dim); squeeze to (hidden_dim,)
    ...
    def extract_features_batch(self, batch_audio_arrays):
        ...
        :param batch_audio_arrays: List of 1D numpy arrays, each representing a preprocessed audio file.
        ...
        # outputs.hidden_states: tuple of tensors each with shape (batch, time, hidden_dim)
        # Stack them to shape: (num_layers, batch, time, hidden_dim)
        all_layer_hidden_states = torch.stack(outputs.hidden_states)
        # Average over the time dimension -> shape: (num_layers, batch, hidden_dim)
        time_reduced_hidden_states = all_layer_hidden_states.mean(dim=-2)
        # Permute to (batch, num_layers, hidden_dim)
    ...
    def extract_folder(self, folder_path, output_file):
        ...
        # Process batches in parallel if requested.
