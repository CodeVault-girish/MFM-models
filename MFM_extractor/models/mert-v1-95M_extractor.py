...
class MertV195Extractor:
    ...
    def extract_features(self, audio_path):
        ...
        all_layer_hidden_states = torch.stack(outputs.hidden_states).squeeze(1)  # shape: (13, time, hidden_dim)
        
        # Average over time dimension -> shape: (13, hidden_dim)
    ...
    def extract_features_batch(self, batch_audio_arrays):
        ...
        # Stack hidden states: outputs.hidden_states is a tuple of tensors of shape (batch, time, hidden_dim)
        # Stack them to get shape: (num_layers, batch, time, hidden_dim)
        all_layer_hidden_states = torch.stack(outputs.hidden_states)
        # Average over the time dimension (dim=2) -> shape: (num_layers, batch, hidden_dim)
        time_reduced_hidden_states = all_layer_hidden_states.mean(dim=2)
        # Permute to shape: (batch, num_layers, hidden_dim)
        time_reduced_hidden_states = time_reduced_hidden_states.permute(1, 0, 2)
        # Pass through aggregator: expects shape (batch, in_channels, seq_len) where in_channels=13
